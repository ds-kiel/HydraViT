...
# 
# Modifications:
# - Adjusted script to measure throughput of a model
...
parser.add_argument('--heads', default=[3, 6, 12], type=int, nargs='+', metavar="HEADS",
...
def validate(args):
    ...
    if '_dyn_' in args.model:
        args.model_kwargs['dim'] = args.head
        print('Set model dim to', args.head)
    model = create_model(
        args.model,
    ...
    print(f'bs_1 = mean: {mean_syn}, std: {std_syn}')
    
    dummy_input = torch.randn(optimal_batch_size, 3, 224, 224).to(device)
...
def main():
    ...
    if len(model_cfgs):
        ...
        if args.retry:
            ...
            print(f'Running validation for model {args.model} with heads {args.heads}')
            heads = [args.heads[i] * 64 for i in range(len(args.heads))]
            for head in heads:
                args.head = head
                results = validate(args)
