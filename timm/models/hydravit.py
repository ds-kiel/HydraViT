...
#
# Modifications:
# - Adjusted DeiT model for HydraViT to have dynamically executable subnetworks
...
from timm.layers import DropPath, trunc_normal_, lecun_normal_, resample_patch_embed, \
    resample_abs_pos_embed, PatchDropout, use_fused_attn, to_2tuple, _assert
...
__all__ = ['HydraViT']  # model_registry will add each entrypoint fn to this
...
class Identity(nn.Module):
    ...
    def forward(self, input: Tensor, p) -> Tensor:
        return input
...
class Linear(nn.Module):
    ...
    def forward(self, input: Tensor, p_in, p_out=-1) -> Tensor:
        if p_out== -1:
            p_out = p_in
        if p_in:
...
class Conv2d(_ConvNd):
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor], p_in, p_out):
        if p_in:
            if self.padding_mode != 'zeros':
                return F.conv2d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                                weight[:p_out], bias[:p_out], self.stride,
                                _pair(0), self.dilation, self.groups)
            return F.conv2d(input, weight[:p_out], bias[:p_out], self.stride,
...
class Attention(nn.Module):
    ...
    def forward(self, x, p):
        ...
        B, N, C = x.shape
        p = p
        qkv = self.qkv(x, p).reshape(B, N, 3, num_heads, p // num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        ...
        x = x.transpose(1, 2).reshape(B, N, p)
        x = self.proj(x, p)
        x = self.proj_drop(x)
        return x
...
class LayerScale(nn.Module):
    ...
    def forward(self, x, p):
        return x.mul_(self.gamma[0:p]) if self.inplace else x * self.gamma[0:p]
...
class Block(nn.Module):
    ...
    def forward(self, x, p):
        x = x[:, :,0:p] + self.drop_path1(self.ls1(self.attn(self.norm1(x, p), p), p))
        x = x[:, :,0:p] + self.drop_path2(self.ls2(self.mlp(self.norm2(x, p), p), p))
        return x
        

class HydraViT(nn.Module):
    """ HydraViT
    Official Implementation of `HydraViT: Stacking Heads for a Scalable ViT` 
        - https://openreview.net/forum?id=kk0Eaunc58
    
    
    Based on implementation of Vision Transformer by Ross Wightman
    ...
    def _pos_embed(self, x):
        ...
        if self.no_embed_class:
            ...
            x = x + pos_embed[:,:,:C]
            if self.cls_token is not None:
                x = torch.cat((self.cls_token[:,:,:C].expand(x.shape[0], -1, -1), x), dim=1)
        else:
...
@torch.no_grad()
def _load_weights(model: HydraViT, checkpoint_path: str, prefix: str = ''):
...
@register_model
def hydravit_dyn_patch16_224(pretrained=False, dim=768, **kwargs) -> HydraViT:
    """ HydraViT model @ 224x224 from paper (https://openreview.net/forum?id=kk0Eaunc58).
    """
    model_args = dict(patch_size=16, embed_dim=dim, depth=12, num_heads=dim//64)
