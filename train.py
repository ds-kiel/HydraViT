...
# 
# Modifications:
# - Adjusted training loop to support stochastic dropout training
...
group.add_argument('--drop-p', type=int, default=-1, metavar='N',
                   help='Operational size of embedding (default: -1)')
...
group.add_argument('--heads', default=[3, 6, 12], type=int, nargs='+', metavar="HEADS",
                   help='list of heads that will be used in the model')
group.add_argument('--weights', type=int, nargs='+', metavar="WEIGHTS",
                   help='list of weights for weighted sampling of the heads (in %)')
...
def main():
    ...
    combs = [(args.heads[i], args.heads[i] * 64) for i in range(len(args.heads))]
    for comb in combs:
        model_ema.module.p = comb[1]
        validate(
            model_ema.module,
            loader_eval,
            validate_loss_fn,
...
def train_one_epoch(
    ...
    if args.weights:
        assert len(args.heads) == len(args.weights)
        assert sum(args.weights) == 100
        weights = [w  / 100 for w in args.weights]
    ...
    for batch_idx, (input, target) in enumerate(loader):
        if args.weights:
            choice = random.choices([i for i in range(len(heads))], weights=weights)[0]
        else:
            choice = random.choices([i for i in range(len(heads))])[0]
        actual_model.p = heads[choice] * 64
